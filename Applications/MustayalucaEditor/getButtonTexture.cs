...
namespace MustayalucaEditor
{
  public partial class getButtonTexture : Form
  {
    ...
    string MustayalucaMediaPath = string.Empty;
    ...
    {
      MustayalucaMediaPath = Path.Combine(SkinInfo.mpPaths.Mustayalucapath, "media");
      iconList();
    ...
    void displayIcons()
    {
      ...
      if (string.IsNullOrEmpty(buttonTexture) || !File.Exists(Path.Combine(MustayalucaMediaPath, buttonTexture)))
        currentIcon = Path.Combine(MustayalucaMediaPath, "homeButtons\\_noimage.png");
      else
        currentIcon = buttonTexture;

      workingImage = Image.FromFile(Path.Combine(MustayalucaMediaPath, currentIcon));
    ...
    {
      string homeButtonPath = Path.Combine(MustayalucaMediaPath, "homebuttons");
      string newIcon = ((PictureBox)sender).Tag.ToString();
    ...
    List<string> iconList()
    {
      ...
      DirectoryInfo dInfo = new DirectoryInfo(Path.Combine(MustayalucaMediaPath, "homebuttons"));
      iconFiles.Clear();
    ...
    private void buttonThemes_Click(object sender, EventArgs e)
    {
      ...
      SelectedIcon = formMustayalucaEditor.menuItems[workingIndex].buttonTexture;
      workingImage = Image.FromFile(Path.Combine(MustayalucaMediaPath, SelectedIcon));
      pbCurrentIcon.Image = workingImage.GetThumbnailImage(128, 128, null, new IntPtr());
      workingImage.Dispose();
    }
  }
