...
namespace MustayalucaEditor
{
  public partial class showButtonTheme : Form
  {
    ...
    void populateScreen(string theme)
    {
      ...
      themePreview.Image = Image.FromFile(Path.Combine(Path.Combine(MustayalucaMediaPath, "homebuttons"), theme + "themepreview.jpg"));
      iconList(theme);
    ...
    private void btSetActiveTheme_Click(object sender, EventArgs e)
    {
      ...
      {
        formMustayalucaEditor.menuItems[i].buttonTexture = formMustayalucaEditor.setDefaultIcons(int.Parse(formMustayalucaEditor.menuItems[i].hyperlink), theTheme);
      }
      this.Close();
    }
