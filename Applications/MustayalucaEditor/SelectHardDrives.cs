...
namespace MustayalucaEditor
{
  public partial class SelectHardDrives : Form
  {
    ...
    public SelectHardDrives()
    {
      ...
      if (!(formMustayalucaEditor.driveFreeSpaceList == "false"))
      {
        lboxAvailableDrives.Items.Clear();
        string[] configuredDrives = formMustayalucaEditor.driveFreeSpaceList.Split(',');
    ...
    {
      formMustayalucaEditor.driveFreeSpaceDrives.Clear();
      foreach (string hd in hardDrives)
      {
        if (!string.IsNullOrEmpty(hd))
        {
