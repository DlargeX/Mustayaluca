...
namespace MustayalucaEditor
{
  class Helper
  {
    ...
    {
      string[] files = System.IO.Directory.GetFiles(SkinInfo.mpPaths.Mustayalucapath);
    ...
    {
      formMustayalucaEditor.prettyItems.Clear();
      XmlDocument doc = new XmlDocument();
      Stream stream = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("MustayalucaEditor.xmlFiles.QuickSelectList.xml");
      try
      {
        doc.Load(stream);
      }
      ...
      {
        showError("Exception while loading QuickSelectList.xml\n\n" + e.Message, formMustayalucaEditor.errorCode.loadError);
        formMustayalucaEditor.basicHomeLoadError = true;
        return;
      }
      XmlNodeList nodeList = doc.DocumentElement.SelectNodes("/items/item");
      foreach (XmlNode node in nodeList)
      {
        formMustayalucaEditor.prettyItem pItem = new formMustayalucaEditor.prettyItem();
        ...
        if (ids.Contains(pItem.id))
          formMustayalucaEditor.prettyItems.Add(pItem);
    ...
    public bool pluginEnabled(Plugins plugin)
    {
      ...
      {
        showError("Error reading MediaPortal.xml : " + e.Message, formMustayalucaEditor.errorCode.readError);        
      }
    ...
    public DialogResult showError(string errorText, formMustayalucaEditor.errorCode errorType)
    {
      DialogResult rtype = new DialogResult();
      switch (errorType)
      {
        case formMustayalucaEditor.errorCode.info:
          ...
          break;
        case formMustayalucaEditor.errorCode.infoQuestion:
          return MessageBox.Show(errorText,
              "Continue Editing",
              MessageBoxButtons.YesNo,
              MessageBoxIcon.Question,
              MessageBoxDefaultButton.Button1);
        case formMustayalucaEditor.errorCode.loadError:
          MessageBox.Show("Error loading menu, file seems invalid\r\rReason: " + errorText + "  ",
              ...
              MessageBoxDefaultButton.Button1);
          break;
        case formMustayalucaEditor.errorCode.readError:
          MessageBox.Show("Error loading menu, file seems invalid\r\rReason: " + errorText + "  ",
              "Click OK to continue.",
              MessageBoxButtons.OK,
              MessageBoxIcon.Error,
              MessageBoxDefaultButton.Button1);
          break;
        case formMustayalucaEditor.errorCode.major:
    ...
    public void writeMPConfiguration(string sectionName, string entryName, string entryValue)
    {
      ...
      {
        showError("Error writing MediaPortal.xml : " + e.Message, formMustayalucaEditor.errorCode.readError);
      }
