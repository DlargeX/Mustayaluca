...
namespace MustayalucaEditor
{
  public partial class formMustayalucaEditor
  {
    ...
    private void installAnimatedIcons_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
    {
      if (!downloadActive)
      {
        optionDownloadURL = "http://Mustayaluca.googlecode.com/files/Mustayaluca_V1.0_AnimatedWeatherIcons.zip";
        optionDownloadPath = Path.Combine(Path.GetTempPath(),"Mustayaluca_V1.0_AnimatedWeatherIcons.zip");
    ...
    private void installWeatherBackgrounds_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
    {
      if (!downloadActive)
      {
        optionDownloadURL = "http://Mustayaluca.googlecode.com/files/Mustayaluca_V1.0_LinkedWeatherBackgrounds.zip";
        optionDownloadPath = Path.Combine(Path.GetTempPath(),"Mustayaluca_V1.0_LinkedWeatherBackgrounds.zip");
        destinationPath = SkinInfo.mpPaths.skinBasePath;
