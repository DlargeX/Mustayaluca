...
namespace MustayalucaConfig
{
  public class MiscConfigGUI : GUIWindow
  {
    ...
    {
      //MustayalucaConfig.SetProperty("#Mustayaluca.ActionMenu.Image", MiscConfigGUI.ShowHiddenMenuImage ? "Action_menu.png" : "-");
      //MustayalucaConfig.SetProperty("#Mustayaluca.RoundedPosters.Image", MiscConfigGUI.ShowRoundedImages ? "round.poster.frame.png" : "-");

      //// Icons in Artwork require a few image properties
      //MustayalucaConfig.SetProperty("#Mustayaluca.Icons.Background", MiscConfigGUI.ShowIconsInArtwork ? "overlaywubg.png" : "-");
      //MustayalucaConfig.SetProperty("#Mustayaluca.Icons.Watched", MiscConfigGUI.ShowIconsInArtwork ? "overlaywatched.png" : "-");
      //MustayalucaConfig.SetProperty("#Mustayaluca.Icons.UnWatched", MiscConfigGUI.ShowIconsInArtwork ? "overlayunwatched.png" : "-");
      //MustayalucaConfig.SetProperty("#Mustayaluca.Icons.NAWatched", MiscConfigGUI.ShowIconsInArtwork ? "overlaywatchedna.png" : "-");
      //MustayalucaConfig.SetProperty("#Mustayaluca.Icons.NAUnWatched", MiscConfigGUI.ShowIconsInArtwork ? "overlayunwatchedna.png" : "-");
    }
    ...
    private bool ShowColorSelectorMenu(string currentColor, out string chosenColor)
    {
      ...
      switch (dlg.SelectedId)
      {
        case 1:
          ...
          while (!validColor)
          {
            if (MustayalucaConfig.ShowKeyboard(currentColor, out retValue))
            {
    ...
    private string GetColorTextureFromMemory(Size size, string identifer)
    {
      try
      {
        ...
        string textureIdentifer = string.Concat("[#Mustayaluca.", identifer, "]");
    ...
    {      
      ...
      switch (dlg.SelectedId)
      {
        ...
        case 6:
          ...
          WatchedColorTemp = "A8F7A8";
          RemoteColorTemp = "F7C3A8";
          break;
      }
    ...
    private void ShowUnfocusedAlphaMenu()
    {
      ...
      switch (dlg.SelectedId)
      {
        case 1:
          ...
          if (MustayalucaConfig.ShowKeyboard(input, out retValue))
          {
            int output = 0;
        ...
        case 3:
          UnfocusedAlphaThumbsTemp = "160";
          UnfocusedAlphaListTemp = "160";
          break;
      }
    ...
    private void ApplyConfigurationChanges()
    {
      ...
      //  string sourceFile = Path.Combine(SkinInfo.mpPaths.Mustayalucapath, UseLargeFonts ? "fonts.large.xml" : "fonts.normal.xml");
      //  string destinationFile = Path.Combine(SkinInfo.mpPaths.Mustayalucapath, "fonts.xml");
      //  try
      //  {
      //    smcLog.WriteLog("Setting Font Size to: {0}", UseLargeFonts ? "Large" : "Normal");
      //    File.Copy(sourceFile, destinationFile, true);
      //    MustayalucaConfig.PendingRestartChanges.Add(MustayalucaConfig.PendingChanges.ClearCache);
      //  }
      //  catch (Exception ex)
      //  {
      ...
      if (requiresRestart)
      {
        MustayalucaConfig.ShowRestartMessage(GetID, Translation.MiscMenu);
      }
    ...
    public override int GetID
    {
      get
      { 
        return (int)MustayalucaConfig.SMPScreenID.SMPMiscSettings; 
      }    
    }

    public override bool Init()
    {      
      return Load(GUIGraphicsContext.Skin + @"\MustayalucaConfig_misc.xml");
    }
